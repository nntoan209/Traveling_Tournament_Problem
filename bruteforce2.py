...
def heuristic(N,cost_matrix):
    c_min = float('inf')
    for i in range(N):
        for j in range(i+1,N):
            if cost_matrix[i][j] < c_min:
...
def find_all(N,k =0,res = [],tmp = []): 
    global f_opt,solution,f_recent,count
    count += 1
    if k == 2 *N -2:
        if f_recent < f_opt:
            f_opt = f_recent
        ...
        for week in rs:
            ...
            h_score = heuristic(N,cost_matrix) *(N//2)*(2*N-2-k)
            if f_recent + h_score < f_opt:
                find_all(N,k)
